...
#define OPENWEATHERMAP_WEATHER_API_URL "http://api.openweathermap.org/data/2.5/weather"
#endif
...
/**
 * Two options for APIs to use. In germany use accurate deutsche wetterdienst wrapper (brightsky api)
 * Internationally use openweathermap (which needs a free api key)
 */
enum API_OPTIONS
{
  DWD_BRIGHTSKY = 0,
...
uint16_t mode_clear_sky()
{
  ...
  // Add a "sun" effect that moves slowly
  for (int i = 0; i < 20; i++)
  {
    int pos = (clearSunPosition + i) % SEGMENT.length();
    SEGMENT.setPixelColor(pos, 0x808010);
  }
...
uint16_t mode_partly_cloudy()
{
  ...
  for (int i = 0; i < numBlueSpots; i++)
  {
    int spotSize = 5; // Fixed size for more stability
    for (int j = 0; j < spotSize; j++)
    {
      int pos = (blueSpotPositions[i] + j) % SEGMENT.length();
      SEGMENT.setPixelColor(pos, CRGB::DeepSkyBlue);
    }
  }
...
{
  // Darker grey background (R:8, G:8, B:12)
  uint32_t backgroundColor = 0x08080C;
  uint32_t rainColor = 0x06298A;

  // Set background for all pixels
  for (int i = 0; i < SEGMENT.length(); i++)
  {
    SEGMENT.setPixelColor(i, backgroundColor);
  }

  // Create raindrops with isolation zones
  for (int i = 0; i < SEGMENT.length(); i++)
  {
    if (random8() < 18)
    { // 7.5% chance
      // Create darker buffer zones around drop (if possible)
      if (i > 3)
        SEGMENT.setPixelColor(i - 3, 0x040406); // Very dark grey
      if (i > 4)
        SEGMENT.setPixelColor(i - 4, backgroundColor);
      if (i < SEGMENT.length() - 3)
        SEGMENT.setPixelColor(i + 3, 0x040406);
      if (i < SEGMENT.length() - 4)
        SEGMENT.setPixelColor(i + 4, backgroundColor);

      // Main drop
      uint8_t mainDrop = random8(120, 180);
      SEGMENT.setPixelColor(i, color_blend(backgroundColor, rainColor, mainDrop));

      // Trails with buffer zones
      if (i > 0)
      {
        uint8_t trail1 = random8(40, 80);
        SEGMENT.setPixelColor(i - 1, color_blend(backgroundColor, rainColor, trail1));
      }
      if (i > 1)
      {
        uint8_t trail2 = random8(20, 40);
        SEGMENT.setPixelColor(i - 2, color_blend(backgroundColor, rainColor, trail2));
      }
    }
  }

  uint16_t frameDelay = map(SEGMENT.speed, 0, 255, 1500, 200);
  return frameDelay;
}

#define NUM_SNOWFLAKES 4
...
void initSnowflakes(uint16_t numLeds)
{
  for (int i = 0; i < NUM_SNOWFLAKES; i++)
  {
    ...
    snowflakes[i].speed = 0.02 + 0.07 * (random8() / 255.0); // Speed between 0.02 and 0.09
  }
}
...
uint16_t mode_snow(void)
{
  ...
  for (int i = 0; i < NUM_SNOWFLAKES; i++)
  {
    uint16_t ledPos = (uint16_t)snowflakes[i].pos;
    // Draw snowflake as 2 pixels wide for visibility
    SEGMENT.setPixelColor(ledPos, 0xFFFFFF); // Bright white snowflake
    if (ledPos + 1 < SEGMENT.length())
      SEGMENT.setPixelColor(ledPos + 1, 0xA0C8FF); // Fainter blue-white edge

    snowflakes[i].pos += snowflakes[i].speed;
    if (snowflakes[i].pos >= SEGMENT.length())
    {
      snowflakes[i].pos = 0;
      snowflakes[i].speed = 0.02 + 0.07 * (random8() / 255.0);
    }
  }
...
uint16_t mode_thunderstorm()
{
  ...
  for (uint16_t i = 0; i < SEGMENT.length(); i++)
  {
    uint8_t flicker = random8((uint16_t)(i * 31 + frame * 17)) % 10;                                 // pseudo-random per pixel per frame
    SEGMENT.setPixelColor(i, ((0x00 + flicker) << 16) | ((0x00 + flicker) << 8) | (0x30 + flicker)); // dark blue
  }
  ...
  if (globalFlash > 0)
  {
    for (uint16_t i = 0; i < SEGMENT.length(); i++)
    {
      SEGMENT.setPixelColor(i, color_blend(SEGMENT.getPixelColor(i), 0xFFFFFF, globalFlash));
    }
  ...
  if (now - lightningBolt.lastTime > lightningBolt.interval)
  {
    if (boltPause)
    {
      ...
      lightningBolt.interval = 400 + random16(1200); // Normal interval for next strike (0.4-1.6s)
    }
    else if (boltStrikesLeft == 0)
    {
      ...
      boltPause = true;
      lightningBolt.interval = 800 + random16(2000); // Pause between positions (0.8-2.8s)
    }
    else
    {
  ...
  {
    for (uint16_t i = lightningBolt.start; i < lightningBolt.start + lightningBolt.len && i < SEGMENT.length(); i++)
    {
      SEGMENT.setPixelColor(i, color_blend(SEGMENT.getPixelColor(i), 0xFFFFCC, 255 - lightningBolt.fade));
      if (i > 0)
        SEGMENT.setPixelColor(i - 1, color_blend(SEGMENT.getPixelColor(i - 1), 0xA0A0FF, 128 - lightningBolt.fade / 2));
...
void initWindSpots(uint16_t numLeds)
{
  ...
  {
    windSpots[i].pos = random(numLeds);
    windSpots[i].speed = 0.2 + 0.15 * (random8() / 255.0); // Speed between 0.2 and 0.35
    windSpots[i].width = 3 + (random8() % 2);              // Width 3-4
  }
}
...
uint16_t mode_wind(void)
{
  ...
  // 1. Fill all LEDs with a black for good contrast
  for (uint16_t i = 0; i < SEGMENT.length(); i++)
  {
    // previous ai idea of dim blue grey sky
    // SEGMENT.setPixelColor(i, 0x101018); // Dim blue-grey
    SEGMENT.setPixelColor(i, BLACK); // Black to have good contrast for clouds
  }

  // 2. Move and draw wind spots
  for (int s = 0; s < NUM_WIND_SPOTS; s++)
  {
    ...
    // Brighter soft-edged spot
    for (int w = -windSpots[s].width; w <= windSpots[s].width; w++)
    {
      int led = ((int)windSpots[s].pos + w + SEGMENT.length()) % SEGMENT.length();
      // Brighter: center is 170, edges are dimmer
      uint8_t brightness = 170 - abs(w) * (170 / (windSpots[s].width + 1));
      uint32_t col = ((brightness << 16) | (brightness << 8) | (brightness)); // grey
      SEGMENT.setPixelColor(led, color_blend(SEGMENT.getPixelColor(led), col, brightness));
    }
  }
...
#define NUM_STARS 6 // Adjust as needed
...
void initStars(uint16_t numLeds)
{
  ...
  {
    stars[i].pos = random16(numLeds);
    stars[i].phase = random8();
    stars[i].speed = random8(2, 8); // Different twinkle speeds
  }
}
...
uint16_t mode_night(void)
{
  ...
  // Fade all LEDs to black manually
  for (uint16_t i = 0; i < SEGMENT.length(); i++)
  {
    uint32_t col = SEGMENT.getPixelColor(i);
    // Extract RGB
    uint8_t r = (col >> 16) & 0xFF;
    uint8_t g = (col >> 8) & 0xFF;
    uint8_t b = col & 0xFF;
    // Fade by 32 (adjust as needed)
    r = (r > 32) ? r - 32 : 0;
    g = (g > 32) ? g - 32 : 0;
    b = (b > 32) ? b - 32 : 0;
    SEGMENT.setPixelColor(i, (r << 16) | (g << 8) | b);
  }
  ...
  for (int i = 0; i < NUM_STARS; i++)
  {
    ...
    uint8_t brightness = sin8(stars[i].phase);
    uint8_t max_brightness = 35; // Adjust as needed (0-255)
    brightness = (brightness * max_brightness) / 255;
    if (brightness < 10 && random8() < 10)
    {
      stars[i].pos = random16(SEGMENT.length());
      stars[i].phase = random8();
      stars[i].speed = random8(2, 8);
    }
    SEGMENT.setPixelColor(stars[i].pos, color_blend(CRGB::Black, WHITE, brightness));
  }
...
uint16_t mode_fog(void)
{
  ...
  for (int f = 0; f < NUM_FOG_BANKS; f++)
  {
    ...
    {
      int led = ((int)fogBanks[f].pos + w + SEGMENT.length()) % SEGMENT.length();
      // Very soft edge: center is 60, edges fade out
      uint8_t brightness = 60 - abs(w) * (60 / (fogBanks[f].width + 1));
      uint32_t col = ((brightness << 16) | (brightness << 8) | (brightness)); // soft gray
      SEGMENT.setPixelColor(led, color_blend(SEGMENT.getPixelColor(led), col, brightness));
    }
  }
...
void initHailstones(uint16_t numLeds)
{
  for (int i = 0; i < NUM_HAILSTONES; i++)
  {
    ...
    hailstones[i].speed = 0.25 + 0.25 * (random8() / 255.0); // Fast fall
    hailstones[i].width = 1 + (random8() % 2);               // 1-3 pixels wide
  }
}
...
uint16_t mode_hail(void)
{
  ...
  for (int h = 0; h < NUM_HAILSTONES; h++)
  {
    ...
    {
      hailstones[h].pos = 0;
      hailstones[h].speed = 0.25 + 0.25 * (random8() / 255.0);
      hailstones[h].width = 2 + (random8() % 2);
    }
    for (int w = 0; w < hailstones[h].width; w++)
    {
      int led = ((int)hailstones[h].pos + w) % SEGMENT.length();
      SEGMENT.setPixelColor(led, 0x2162A3); // Blue hailstone
    }
  }
...
/**
 * The all encompassing weather effect mode that picks the current weather on itself
 */
...
private:
  ...
  void updateWeather()
  {
    ...
    if (API_USED == API_OPTIONS::OPENWEATHERMAP)
    {
      ...
      String owmUrl = String(OPENWEATHERMAP_WEATHER_API_URL) + "?lat=" + LAT + "&lon=" + LON + "&appid=" + OPENWEATHERMAP_API_KEY + "&units=metric";
      ...
      if (httpCode == 200)
      {
        ...
        DEBUG_PRINTLN(doc["dt"].as<int32_t>() > doc["sys"]["sunset"].as<int32_t>());
        DEBUG_PRINTLN(doc["dt"].as<int32_t>() < doc["sys"]["sunrise"].as<int32_t>());
        DEBUG_PRINTLN(F("Should have shown all that stuff"));
        if (doc["dt"].as<int32_t>() > doc["sys"]["sunset"].as<int32_t>() || doc["dt"].as<int32_t>() < doc["sys"]["sunrise"].as<int32_t>())
        {
          isNight = true;
        }
        ...
        setAnimationFromOpenWeatherData(currentWeather, windSpeed, isNight, cloudiness);
    ...
    else if (API_USED == API_OPTIONS::DWD_BRIGHTSKY)
    {
      ...
      String brightskyUrl = String(DWD_WEATHER_API_URL) + "?lat=" + LAT + "&lon=" + LON;
  ...
  void setAnimationFromOpenWeatherData(String weather, float windSpeed, bool isNight, int cloudiness)
  {
...
public:
  ...
  void setup()
  {
    ...
    strip.addEffect(WeatherAnimation::ANIM_CLEAR, &mode_clear_sky, "WeatherClearSky@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_PARTLY_CLOUDY, &mode_partly_cloudy, "WeatherPartlyCloudy@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_CLOUDY, &mode_cloudy, "WeatherCloudy@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_RAIN, &mode_raining, "WeatherRain@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_SNOW, &mode_snow, "WeatherSnow@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_NIGHT, &mode_night, "WeatherNight@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_WIND, &mode_wind, "WeatherWind@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_THUNDERSTORM, &mode_thunderstorm, "WeatherThunderstorm@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_FOG, &mode_fog, "WeatherFog@!;<empty>;;01");
    strip.addEffect(WeatherAnimation::ANIM_HAIL, &mode_hail, "WeatherHail@!;<empty>;;01");
  ...
  /****** Config page stuff */
  // from https://github.com/Aircoookie/WLED/blob/main/usermods/EXAMPLE_v2/usermod_v2_example.h
  ...
  bool readFromConfig(JsonObject &root) override
  {
    ...
    configComplete &= getJsonValue(top["openWeatherMap_ApiKey"], OPENWEATHERMAP_API_KEY);
